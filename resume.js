$(document).ready(function () {
    ...
    $(window).scroll(function(){
        if($(window).scrollTop()>=$('.p1').offset().top && $(window).scrollTop()<$('.section1').offset().top){
    ...
    //判斷滾輪是否到底//
    $(window).on('scroll', function() {
        if ($(window).scrollTop() >= $('.section4').offset().top + $('.section4').outerHeight() - window.innerHeight) {
            $('.nav li').css('background-color','white')
    ...
    /*$(".nav li:eq(2)").click(function(){
        $("html,body").animate({"scrollTop":"1180px"})
    })
    $(".nav li:eq(3)").click(function(){
        $("html,body").animate({"scrollTop":"1900px"})
    })
    $(".nav li:eq(4)").click(function(){
        $("html,body").animate({"scrollTop":"2700px"})
    })*/
